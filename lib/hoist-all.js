...
function createSymlink(src, dest, entry, {config, modConfig}) {
  ...
  try {
    ...
    if (stat.isSymbolicLink()) {
      ...
      target = path.resolve(src, (entry.startsWith('@') || entry.startsWith('.bin')) ? '..' : '.', target);
      ...
      if (process.platform === 'win32') {
        // dont even try file symlink on windows
...
const disableProgress = !isInteractive;
...
function copyModulesTopLevel(srcNodeModules, destNodeModules, skipNames = new Set(), config = {}) {
  ...
  for (const entry of entries) {
    if ((entry.startsWith('.') && !entry.startsWith('.bin')) || SKIP_FOLDERS.has(entry) || skipNames.has(entry)) {
      continue;
    }
    ...
    if ((entry.startsWith('@') || entry.startsWith('.bin')) && entryStat.isDirectory()) {
      const scopedChildren = fs.readdirSync(srcEntryPath);
      for (const child of scopedChildren) {
        if ((child.startsWith('.') && !child.startsWith('.bin')) || SKIP_FOLDERS.has(child) || skipNames.has(child) || skipNames.has(`${entry}/${child}`)) {
          continue;
        }
  ...
  for (const mod of modulesToCopy) {
    ...
    if (task === 'symlink')
      createSymlink(mod.src, mod.dest, mod.name, {config, modConfig});
    else if (task === 'copy')
 ...
 */
function hoistAll({workspaceRoot = process.cwd(), appsFolder = 'apps', app: onlyApp}) {
  ...
  for (const app of apps) {
    ...
    if (config && (config.target === 'app' || config.target === 'both')) {
